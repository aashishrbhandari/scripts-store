...
#
#### Step2: Get Each NIC TX(Transmitted Traffic/Send from this NIC) & RX(Received On this NIC)
...
do
    ...
    SPEED_RX_CALC=$(bytesToHR ${SPEED_RX});
    SPEED_TX_CALC=$(bytesToHR ${SPEED_TX});

    TOTAL_SPEED=$(( ${SPEED_RX} + ${SPEED_TX} ));
    TOTAL_SPEED_CALC=$(bytesToHR ${TOTAL_SPEED});
