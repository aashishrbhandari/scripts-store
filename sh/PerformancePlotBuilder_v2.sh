...
PERFORMANCE_PLOTS_PATH=${4?Error: Please Specify the 4th Argument as Path where the Performace Plot(PNG) file will be Saved}

PERFORMANCE_LOG_FILE=${5:-/var/log/safesquid/performance/performance.log}


### Extended v1 -> v2

TEMP_EXTRACTED_PERF_LOG_PATH="/var/log/temp_logs";

mkdir -p ${TEMP_EXTRACTED_PERF_LOG_PATH};

TEMP_EXTRACTED_PERF_LOG_FILE="${TEMP_EXTRACTED_PERF_LOG_PATH}/Perf_Log_${START_DURATION}_${END_DURATION}.log";

START_DAY=${START_DURATION::-6}
END_DAY=${END_DURATION::-6}

if [[ ${START_DAY} == ${END_DAY} ]];
then
	BETWEEN_DATES=${START_DAY}
else
	BETWEEN_DATES=$(d=; n=0; until [ "$d" = "$END_DAY" ]; do ((n++)); d=$(date -d "$START_DAY + $n days" +%Y%m%d); echo $d; done)
fi


for ONE_DATE in ${BETWEEN_DATES};
do	
	if [[ ${LOG_GREP_STRING} == "" ]];
	then
		LOG_GREP_STRING="${ONE_DATE}";
	else
		LOG_GREP_STRING="${LOG_GREP_STRING}|${ONE_DATE}";
	fi
done

# GLOB to Take only Performance.log into Account
zgrep -iEah ${LOG_GREP_STRING} ${PERFORMANCE_LOG_FILE} > ${TEMP_EXTRACTED_PERF_LOG_FILE};

PERFORMANCE_LOG_FILE=${TEMP_EXTRACTED_PERF_LOG_FILE};
...
echo "[PERFORMANCE_FILE_NAME: ${PERFORMANCE_FILE_NAME}], [PERFORMANCE_LOG_FILE: ${PERFORMANCE_LOG_FILE}], [PERFORMANCE_PLOTS_PATH: ${PERFORMANCE_PLOTS_PATH}]";
...
set output "${PERFORMANCE_PLOTS_PATH}/${OUTPUT}"
